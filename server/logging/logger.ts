...
export class Logger {
  ...
  static create(cfg: LogConfig, file?: string): Logger {
    const target = file ?? cfg.file;
    const sinks: LogSink[] = [];
    if (target) {
      sinks.push(FileSink.open(target, cfg.max_size_mb, cfg.max_files));
