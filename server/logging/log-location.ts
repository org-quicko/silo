...
export class LogLocation {
  ...
  static forDetached(cfg: Config): string {
    return cfg.log.file ?? path.join(cfg.storage.path, LogLocation.Name);
  }
   ...
   */
  static forReading(cfg: Config, state: RunState | null): string | undefined {
    return state?.log ?? cfg.log.file ?? undefined;
  }
}
