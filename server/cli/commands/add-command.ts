...
export class AddCommand {
  static async run(
    ...
    const result = await PluginInstaller.install({
      pluginsDir: PluginRegistry.directory(cfg),
      spec,
      ref: options.ref,
  ...
  private static async register(
    ...
    const config = AddCommand.entry(result, options);
    const block = PluginBlockWriter.render(config);
    ...
    if (!(await AddCommand.granted(config, result, !!options.yes))) {
  ...
  private static async granted(
    ...
    const missing = result.manifest.claims.filter((claim) => !Claims.has(config.claims, claim as any));
    if (missing.length > 0) {
    ...
    if (config.claims.length === 0) return true;
    ...
    for (const claim of config.claims) console.log(`  ${claim}`);
  ...
  private static entry(result: InstallResult, options: AddOptions): PluginConfig {
    ...
    const config = PluginBlockWriter.defaults(result.name, requested);
    ...
    if (options["on-error"] === "skip" || options["on-error"] === "fail") {
      config.on_error = options["on-error"];
    }
    return config;
  }
