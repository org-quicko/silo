...
export class StatusCommand {
  static async run(cfg: Config): Promise<void> {
    const state = await RunFile.read(cfg.storage.path);

    if (!state) {
      console.log(`silo is not running (${cfg.storage.path})`);
      process.exit(1);
    }

    if (!Daemon.isAlive(state.pid)) {
      console.log(
        `silo is not running (${cfg.storage.path}) — pid ${state.pid} is gone, ` +
