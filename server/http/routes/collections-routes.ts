...
export class CollectionsRoutes {
  static register(app: any, svc: Service) {
    const listHandler = async (c: Context) => {
      ...
      const cols = await svc.listCollections(scope);
    ...
    const createHandler = async (c: Context) => {
      ...
      const col = await svc.putSchema(scope, body.name, body.schema);
    ...
    const putSchemaHandler = async (c: Context) => {
      ...
      const current = await svc.getCollection(scope, name);
      RouteAuth.requireCollectionClaim(
        c,
        scope.project,
        scope.env,
      ...
      const col = await svc.putSchema(scope, name, schema);
      return c.json(col);
    };
    ...
    const deleteHandler = async (c: Context) => {
      ...
      await svc.deleteCollection(scope, name, force);
