...
export class ProjectsRoutes {
  ...
  static register(app: any, svc: Service) {
    ...
    app.get("/api/projects", async (c: Context) => {
      ...
      if (!key) {
        const publicScopes = await svc.publicScopes();
    ...
    app.post("/api/projects", async (c: Context) => {
      ...
      await svc.createProject(project);
    ...
    app.delete("/api/projects/:project", async (c: Context) => {
      ...
      await svc.deleteProject(project, force);
    ...
    ProjectsRoutes.both(app, "get", "", async (c: Context) => {
      const project = c.req.param("project") || "";
      Scope.validateProject(project);
      const envs = await svc.listEnvironments(project);
      const key = c.get("keyInfo") as KeyInfo | undefined;
      if (!key) {
        const publicEnvs = (await svc.publicScopes()).get(project);
    ...
    ProjectsRoutes.both(app, "post", "", async (c: Context) => {
      ...
      await svc.createEnvironment(scope.project, scope.env);
    ...
    ProjectsRoutes.both(app, "delete", "/:env", async (c: Context) => {
      ...
      const force = c.req.query("force") === "true";
      await svc.deleteEnvironment(scope.project, scope.env, force);
      return c.body(null, 204);
    });
  }
