...
import { SqliteSearcher } from "../../adapters/storage/sqlite/sqlite-searcher";
import { SearchIndex } from "../../adapters/storage/sqlite/search-index";
...
describe("SQLite FTS5 searcher", () => {
  ...
  const titles = async (q: string, access: any = everything, extra: any = {}) => {
    const res = await searcher.search({ q, limit: 50, offset: 0, ...extra }, access);
    return { titles: res.items.map((h) => h.entry.data.title), total: res.total, res };
  };
  ...
  describe("rebuild and integrity", () => {
    test("reindex fills an index that was emptied underneath it", async () => {
      ...
      (store as any).db.exec(`DELETE FROM ${SearchIndex.Documents}`);
    ...
    test("the second check sees drift the built-in one cannot", async () => {
      ...
      (store as any).db.exec(`DELETE FROM entries`);
