...
describe("search API", () => {
  ...
  beforeEach(async () => {
    ...
    await svc.putSchema(prod, "pages", { type: "object" });
    await svc.putSchema(staging, "posts", { type: "object" });
    await svc.putSchema(acme, "posts", { type: "object", "x-silo-auth": true });

    await svc.createEntry(prod, "posts", {
    ...
    await svc.createEntry(prod, "pages", { title: "About", body: "Our pricing philosophy." });
    await svc.createEntry(staging, "posts", { title: "Staging pricing draft" });
    await svc.createEntry(acme, "posts", { title: "Acme pricing" });
  ...
  describe("access is compiled before the query", () => {
    test("a key sees only what its claims cover, and total agrees", async () => {
      const { secret } = await svc.createKey("scoped", [
    ...
    test("a wildcard claim narrows to the reach rather than escaping it", async () => {
      const { secret } = await svc.createKey("wild", [
    ...
    test("a key with no entry-read claim finds nothing at all", async () => {
      const { secret } = await svc.createKey("schema-only", [
  ...
  describe("rejections", () => {
    test("a malformed path is a 400, from the search route too", async () => {
      ...
      const res = await app.request(`/api/search?filter=${filter}`, {
        headers: { Authorization: `Bearer ${rootKey}` },
      });
      expect(res.status).toBe(400);
      expect(JSON.stringify(await res.json())).toContain("recursive-descent selectors");
    });

    test("a malformed scope id is refused at the boundary", async () => {
      const res = await app.request("/api/projects/Bad_Project/environments/prod/search?q=x", {
        headers: { Authorization: `Bearer ${rootKey}` },
      });
      expect(res.status).toBe(400);
    });
