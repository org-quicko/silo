...
describe("Projects API", () => {
  ...
  test("GET /api/projects and GET /api/projects/:project/environments list projects and envs", async () => {
    ...
    await service.putSchema(scopeAcmeProd, "posts", { type: "object", "x-silo-auth": true });
    await service.putSchema(scopeAcmeDev, "posts", { type: "object", "x-silo-auth": false });
    await service.createProject("empty");
    await service.createEnvironment("empty", "staging");
    ...
    const { secret: acmeKey } = await service.createKey("acme key", [
  ...
  test("POST /api/projects and POST /api/projects/:project/environments creates projects and envs", async () => {
    const { secret: creatorKey } = await service.createKey("creator", [
    ...
    expect(await res.json()).toMatchObject({ id: "myproject" });
  ...
  test("POST /api/projects rejects unauthorized or invalid requests", async () => {
    const { secret: unauthKey } = await service.createKey("unauth", [
  ...
  test("DELETE /api/projects/:project/environments/:env deletes environment and collections", async () => {
    ...
    await service.createProject(scope.project);
    await service.createEnvironment(scope.project, scope.env);
    await service.putSchema(scope, "notes", { type: "object" });
    await service.createEntry(scope, "notes", { content: "hello" });
  ...
  test("distinguishes projects and envs with underscores", async () => {
    await service.createProject("a_b");
    await service.createEnvironment("a_b", "c");
    await service.createProject("a");
    await service.createEnvironment("a", "b_c");

    const scopes = await service.listScopes();
    expect(scopes.map((s) => s.key()).sort()).toEqual(["a/b_c", "a_b/c"]);

    // Deleting one must not take the other with it.
    await service.deleteEnvironment("a_b", "c", false);
    expect((await service.listScopes()).map((s) => s.key())).toEqual(["a/b_c"]);
  });
  ...
  test("DELETE without force refuses up front instead of half-deleting", async () => {
    ...
    await service.createProject(scope.project);
    await service.createEnvironment(scope.project, scope.env);
    await service.putSchema(scope, "aaa", { type: "object" });
    await service.putSchema(scope, "bbb", { type: "object" });
    await service.createEntry(scope, "bbb", { x: 1 });

    const res = await app.request("/api/projects/proj/environments/env", {
      method: "DELETE",
    ...
    const remaining = (await service.listCollections(scope)).map((c) => c.name);
  ...
  test("DELETE with force erases entry-only collections that have no schema", async () => {
    ...
    const rootHeaders = { Authorization: `Bearer ${rootKey}` };
    const res = await app.request("/api/projects/orph/environments/env?force=true", {
      method: "DELETE",
      headers: rootHeaders,
    });
  ...
  test("a registered empty project survives an export/import round trip", async () => {
    await service.createProject("empty");
    await service.createEnvironment("empty", "staging");
    await service.putSchema(Scope.of("acme", "prod"), "posts", { type: "object" });
    ...
    try {
      const destService = new Service(destStore, { mediaDir: path.join(destDir, "media") });
      await Importer.importDir(destStore, archive, { mode: "replace" });

      expect((await destService.listScopes()).map((s) => s.key()).sort()).toEqual([
        "acme/prod",
        "empty/staging",
      ]);
    } finally {
  ...
  // has no collection-name check (only `Service.putSchema` does), and the
  // import path calls it directly — so an archive carrying
  // `schemas/_secret.schema.json` plants a system-named schema in a user
  // scope. `Service.listCollections` hides `_`-prefixed names, so building the
  ...
  test("DELETE erases system-named collections planted in a user scope", async () => {
    ...
    expect((await store.listScopes()).map((s) => s.key())).toContain("ghosty/prod");
    expect(await service.listCollections(scope)).toEqual([]);
    ...
    const res = await app.request("/api/projects/ghosty/environments/prod?force=true", {
      method: "DELETE",
      headers: rootHeaders,
    });
    expect(res.status).toBe(204);
  ...
  test("startup refuses a default project or env the API could never address", async () => {
    ...
    await expect(service.initDefaults("default", "PROD")).rejects.toThrow(/invalid default scope/);
    await expect(service.initDefaults("My Project", "prod")).rejects.toThrow(/invalid default scope/);
    // `_`-prefixed ids are reserved for Scope.System and must not be
    // reachable through configuration either.
    await expect(service.initDefaults("_system", "_system")).rejects.toThrow(/invalid default scope/);
    expect(await service.listProjects()).toEqual([]);

    await service.initDefaults("default", "prod");
    expect(await service.listProjects()).toEqual(["default"]);
    expect(await service.listEnvironments("default")).toEqual(["prod"]);
  });
  ...
  test("DELETE without force refuses a scope holding only empty collections", async () => {
    ...
    for (const name of ["posts", "authors", "tags"]) {
      await service.putSchema(scope, name, { type: "object" });
    }

    const rootHeaders = { Authorization: `Bearer ${rootKey}` };
    const res = await app.request("/api/projects/fresh", { method: "DELETE", headers: rootHeaders });
    ...
    expect((await service.listCollections(scope)).map((c) => c.name)).toEqual([
      "authors",
      "posts",
      "tags",
    ]);
  ...
  test("DELETE project refuses up front rather than emptying earlier envs", async () => {
    ...
    await service.createProject("multi");
    await service.putSchema(Scope.of("multi", "aaa"), "posts", { type: "object" });
    await service.putSchema(Scope.of("multi", "zzz"), "posts", { type: "object" });
    await service.createEntry(Scope.of("multi", "zzz"), "posts", { title: "keep me" });

    const res = await app.request("/api/projects/multi", {
      method: "DELETE",
      headers: { Authorization: `Bearer ${rootKey}` },
    });
    expect(res.status).toBe(409);

    expect((await service.listCollections(Scope.of("multi", "aaa"))).map((c) => c.name)).toEqual([
      "posts",
    ]);
    expect((await service.listEntries(Scope.of("multi", "zzz"), "posts", {})).total).toBe(1);
  });

  test("a key sees only the projects its claims name", async () => {
    await service.putSchema(Scope.of("public-site", "prod"), "posts", {
      type: "object",
      "x-silo-auth": true,
    });
    await service.putSchema(Scope.of("secret-client", "prod"), "invoices", {
      type: "object",
      "x-silo-auth": true,
    });
    ...
    const media = await service.createKey("media only", [Claims.MediaRead]);
    expect(await listFor(media.secret)).toEqual([]);

    const scoped = await service.createKey("public-site only", [
  ...
  test("anonymous listings show only projects and envs with a public collection", async () => {
    await service.putSchema(Scope.of("open", "prod"), "posts", { type: "object" });
    await service.putSchema(Scope.of("open", "staging"), "drafts", {
      type: "object",
      "x-silo-auth": true,
    });
    await service.putSchema(Scope.of("closed", "prod"), "invoices", {
      type: "object",
      "x-silo-auth": true,
    });
    ...
    await service.putSchema(Scope.of("closed", "prod"), "invoices", { type: "object" });
