...
import { Scaffold } from "../../../create-silo-plugin/src/scaffold";
import { SiloRange } from "../../../create-silo-plugin/src/silo-range";
import type { ScaffoldOptions } from "../../../create-silo-plugin/src/scaffold-options";

/**
 * A scaffolded plugin, loaded the way `serve` loads one: through
 * `PluginRegistry`, into a real `Worker`, writing through a real `Service`.
 *
...
describe("a scaffolded plugin, loaded the way serve loads one", () => {
  ...
  const scaffoldAndLoad = async (opts: ScaffoldOptions, config: PluginConfig) => {
  ...
  test("the default scaffold loads and rewrites an entry", async () => {
    ...
    expect((await svc.getEntry(scope, "posts", entry.id)).data.slug).toBe("hello-plugin-world");
  }, 30000);
  ...
  test("a scaffold with no config schema loads with no [plugins.config] at all", async () => {
    ...
    const entry = await svc.createEntry(scope, "posts", { title: "No Config Here" });
