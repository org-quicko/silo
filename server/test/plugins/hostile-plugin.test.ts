...
describe("a runaway plugin cannot take the server with it", () => {
  ...
  test("the spinning hook is timed out and the host keeps working", async () => {
    ...
    const first = await svc.createEntry(scope, "posts", { title: "one" });
    ...
    const second = await svc.createEntry(scope, "posts", { title: "two" });
    expect(second.id).toBeTruthy();
    expect((await svc.listEntries(scope, "posts", {})).total).toBe(2);
  }, 30000);

  test("a faulted plugin stays down instead of being restarted into the same wall", async () => {
    await svc.createEntry(scope, "posts", { title: "one" });
    ...
    const started = Date.now();
    await svc.createEntry(scope, "posts", { title: "two" });
