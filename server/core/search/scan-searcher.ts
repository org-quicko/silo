...
export class ScanSearcher implements Searcher {
  ...
  constructor(
    store: Storage,
    opts: { visitLimit?: number; timeBudgetMs?: number } = {}
  ) {
    this.store = store;
    this.visitLimit = opts.visitLimit ?? ScanSearcher.DefaultVisitLimit;
    this.timeBudgetMs = opts.timeBudgetMs ?? ScanSearcher.DefaultTimeBudgetMs;
  }
