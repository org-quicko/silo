...
export class Exporter {
  ...
  private static async exportEntries(st: Storage, dest: string, scope: Scope, colName: string): Promise<number> {
  ...
  private static async exportScope(
    ...
    const schemas = await st.listSchemas(scope);
    ...
    const contentCols = [
      ...new Set([...schemas.keys(), ...(await st.listEntryCollections(scope))]),
    ]
    ...
    for (const colName of contentCols) {
      colCounts[`${scope.key()}/${colName}`] = await Exporter.exportEntries(st, dest, scope, colName);
    }
  ...
  static async exportDir(
    ...
    // (D18).
    const scopes = [...(await st.listScopes()), Scope.System];

    for (const scope of scopes) {
      await Exporter.exportScope(st, dest, scope, opts.withKeys, colCounts);
    }
  ...
  static async exportTarGz(
    ...
    try {
      await Exporter.exportDir(st, tmpDir, opts, blobStorage);
