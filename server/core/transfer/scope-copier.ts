...
export class ScopeCopier {
  static async copy(
    ...
    const scoped = await ScopeCopier.read(st, from, to);
    return Importer.executeImport(st, { manifest, scopes: [scoped] }, opts);
  }
   ...
   */
  private static async read(st: Storage, from: Scope, to: Scope): Promise<ScopedImport> {
    const schemas = await st.listSchemas(from);
    ...
    const names = [
      ...new Set([...schemas.keys(), ...(await st.listEntryCollections(from))]),
    ]
    ...
    for (const name of names) {
      entries.set(name, await ScopeCopier.readEntries(st, from, to, name));
    }
