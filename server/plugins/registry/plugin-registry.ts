 ...
 *
 * Built **explicitly** from `cfg.plugins`, in that array's order, at one wiring
...
export class PluginRegistry {
  ...
  private constructor(runtimes: readonly PluginRuntime[], logger: Logger) {
    ...
    // The null object when nothing is configured, so `Service` has one dispatch
  ...
  static directory(cfg: Config): string {
    return path.join(cfg.storage.path, "plugins");
  }
  ...
  static async load(
    ...
    if (cfg.plugins.length === 0) return PluginRegistry.empty(logger);
