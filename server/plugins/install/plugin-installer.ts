 ...
 *
 * This is the part D31 deferred, and the shape it deferred it into: an
 * installer that changes **nothing** about the contract. It writes a directory
 * that the resolution rule frozen in §13.3 already describes, and everything
 * downstream — `ManifestReader`, `PluginLoader`, `serve` — cannot tell an
 * installed plugin from one someone copied in by hand. That is the property
 * which made shipping this after 1.0 froze the surface safe at all.
 *
 * Two rules govern the order of operations, and both are about what is on disk
 * when something goes wrong:
 *
 *  - **Nothing is executed.** The manifest is read and judged statically
 *    (§13.2), the version range is checked against this binary, and a provider
 *    is refused a reserved driver name — all without importing a line of the
 *    package. No lifecycle script runs at any point (§13.8): there is no hook
 *    here for a package to have opinions about its own installation.
 *  - **Nothing lands until everything passes.** The package is staged inside
 *    the plugins directory, so the final move is a rename rather than a copy
 *    across filesystems, and a failure after the move rolls it back. A
 *    half-installed plugin is the outcome worth the most effort to avoid: it is
 *    exactly the "instance that runs and has quietly stopped enforcing
 *    something" that `PluginLoader` refuses to create.
 */
export class PluginInstaller {
  ...
  private static async manifestJson(dir: string): Promise<any> {
    ...
    try {
      ...
      throw new Error(`cannot read the package's package.json: ${err.message}`);
    }
  }
