 ...
 *
 * §12.8 named a lockfile as part of the installer, and this is the smallest
 * thing that earns the name. It is **a record, not a resolver**: nothing reads
 * it at startup, `serve` still loads exactly what `silo.toml` names, and
 * deleting it breaks nothing. That asymmetry is deliberate — a lockfile that
 * gated loading would put a second source of truth beside the config file,
 * which D31 made the whole management surface.
 *
 * What it buys is the question an operator cannot otherwise answer six months
 * later: *which* version of this plugin is in this directory, where did it
 * come from, and are the bytes the ones that were published. A `[[plugins]]`
 * entry names a directory and says nothing about any of that.
 *
 * It lives in the plugins directory rather than beside `silo.toml` because it
 * describes the directory's contents, and D5 says an instance is a thing you
 * can `cp` — the record has to travel with what it records.
 */
