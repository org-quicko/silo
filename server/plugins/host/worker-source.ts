...
export class WorkerSource {
  static readonly Code = [
    ...
    '  self.postMessage({ t: "rpc", id, method, args });',
    "});",
    "",
    "const revive = (e) => {",
    ...
    "  },",
    "  entries: {",
    '    get:    (scope, collection, id) => rpc("entries.get", [scope, collection, id]),',
    '    list:   (scope, collection, query) => rpc("entries.list", [scope, collection, query || {}]),',
    '    create: (scope, collection, data) => rpc("entries.create", [scope, collection, data]),',
    '    update: (scope, collection, id, data, rev) => rpc("entries.update", [scope, collection, id, data, rev]),',
    '    delete: (scope, collection, id, rev) => rpc("entries.delete", [scope, collection, id, rev]),',
    "  },",
    "});",
    "",
    ...
    "      const value = await fn.call(mod.default, msg.event, ctx);",
    '      self.postMessage({ t: "result", id: msg.id, ok: true, value: value === undefined ? null : value });',
    "    } catch (err) {",
    '      self.postMessage({ t: "result", id: msg.id, ok: false, error: wire(err) });',
    "    }",
    "  }",
    "};",
    "",
