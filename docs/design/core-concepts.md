...
**Schema changes vs existing data** — a collection's schema is **frozen while it holds entries** (D70). `SchemaChangeGuard` compares the incoming bundled document against the stored one and answers `409` with the collection name and its entry count; deleting the entries thaws it. Validation still happens on write only — nothing sweeps or rewrites stored entries — but the freeze is what makes "validated on the way in" a property of the *collection* rather than of the moment each row was written. Without it a collection could hold data its own schema rejects, and a client generating types from that schema would get types the data does not fit.

Only the **validating shape** is frozen. `SchemaShape` (`@silo/shared/schema-shape`) strips `x-silo-auth`, `x-silo-search`, `title`, `description` and `$comment` at every depth before comparing, so making a populated collection public, correcting a search path or fixing a field's label stays an ordinary edit. The list is closed: a keyword nobody classified is frozen, because an edit needlessly refused is recoverable and names its own reason, while an unclassified keyword quietly moving what validates is not. The walk is schema-aware rather than key-name-aware — `title` under `properties` is a field called "title", and stripping it would leave a real field outside the freeze.

Exempt, and why: **replace-mode import** has already emptied the collection, a **first create** has nothing to invalidate, and a **rename** (`SchemaRefRewrite`) changes the name the constraints are written under rather than the constraints. Editing a schema over existing data is **deferred rather than refused forever** — a later release is expected to allow it behind a migration plan that states what happens to the entries.
...
**Types are strict, and strings are in codepoint order (D92).** `eq`, `neq` and `in` match only a node of the value's own JSON type, so `eq true` never matches `1` and `in [null]` matches JSON null. A range operator compares a number with a number and a string with a string, and matches nothing otherwise. Strings compare and sort by Unicode codepoint, the one order every database spells the same (`BINARY`, `COLLATE "C"`). A sort ranks types first — missing and null, numbers, strings, booleans, arrays, objects — and ties arrays and objects within their type. The rules and the disagreements they settled are in §6.1.
