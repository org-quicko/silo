...
6. **Search (Completed)** — D30/§5.5: a `Searcher` port with a portable `ScanSearcher` on every adapter and `SqliteSearcher` (FTS5) where the build has it. Addressing is D29 JSONPath, shared with filters and sort, and the admin UI reads it through collection search, a `⌘K` smart search bar, and a filter builder (§9).
