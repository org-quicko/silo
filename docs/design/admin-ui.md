...
8. **Data transfer** — two pages at two blast radii. Under *Server*: claim-aware export/import panels and direct copy from another running silo (merge/replace, data-only/data-plus-keys). Since D74 all three carry a **scope picker** — projects, then environments, then collections, one pane per level, which is the same browser the server manager uses and now a shared primitive rather than a copy of one. Every box is checked by default, because an empty selection *is* the whole instance. Beside it sits a **media mode** control whose default follows the selection the way the server's own does. The panes are a fixed height and the lists scroll: a column of forty-three collections that grew instead would make the page taller than the choice it is asking for. What the archive covers is then stated as the projects it touches with a **sheet** behind it, since a fact row ellipsises and forty-six rules on one line says nothing. Unchecking the first box has to write the implied selection out in full before it can subtract from it, which is the one place the two spellings of "everything" meet. Import and copy read the D75 progress stream, so a long run reports what it is doing instead of appearing to hang. Under *Environment*: copy from another environment of this instance (D22), preview-then-apply, gated on the scoped claims the copy exercises.
