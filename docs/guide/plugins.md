...
does nothing you could not do yourself: place the directory, list it, and it
runs.

Plugins live in the data directory rather than beside the binary for two
reasons. A packaged binary is root-owned and read-only. And an instance is a
...
| `ui` | Ships an admin panel, drawn in a sandboxed frame with no origin | In the operator's browser |
| `providers` | Implements the storage or blob-storage port, and adds a driver name | In-process, before storage opens |
...
module**, because silo imports it before storage exists while the rest of the
package runs in a worker afterwards.

The built-in adapters register through the same registry, under the reserved
names `sqlite`, `fs` and `s3` that no plugin may take. `[storage] driver` is a
lookup in that registry, so a third-party store is selected exactly the way a
built-in one is.

## Writing one

A plugin needs no build step, because silo transpiles TypeScript itself. It
needs **no dependencies at all**, including on silo.
...
[`create-silo-plugin`](../../packages/create-silo-plugin/) asks what the plugin
is for. It then writes the manifest, a runnable stub per hook you pick, the
`silo:api` type declarations, and the `[[plugins]]` block to paste into
`silo.toml`. Everything below is what it produces, and what to change once you
have it.
...
The `silo` block is the **manifest**, and it is static on purpose.
`silo plugin info` has to show an operator what a package wants *before* any of
its code runs.
...
| `contributes.providers` | Storage or blob drivers, each `{ "port", "driver", "entry" }`. `entry` is required: silo imports a provider before storage exists, so it cannot share the module the worker half runs from |
...
A package must contribute *something*. Otherwise nothing would ever call it, and
the start refuses and says so. silo adds a `hooks:` claim per declared hook, and
`http:route` for declared routes, to the request for you. Writing them out again
...
`silo:api` is a **virtual module**. It has no file on disk and it is not on npm.
...
`ValidationError` in play instead of one per plugin.

For editor support, keep `apps/server/src/plugins/host/silo-api-types.d.ts`
next to your plugin. It is types only and contributes nothing at runtime.
`create-silo-plugin` copies it for you, and a test in this repository holds the
two byte-identical.
...
**Which plugins load, and in what order**, is `silo.toml`. It is the operator's
file, and nothing but a text editor writes it:
...
The array is **ordered, and that order is hook dispatch order**, top to bottom.
There is no priority number to compete over, and no load-order surprise.
...
collection. `silo plugin grant` and the management API change it. The split is
the load-bearing part. If grants lived in config, revoking would need a restart.
If registration lived in the database, whoever could write the database could
execute code.

A listed plugin that nobody has approved is **`pending`**. It loads, it is
delivered nothing, and every `ctx` call is refused. That is a state, not a
failure, and it is the one exception to "a plugin that cannot do its job refuses
the start": approving needs a running server to approve through, so a server
that refused to boot could never be given one. silo is loud about it, on every
start, in `silo plugin list`, and in a non-zero exit from `silo plugin doctor`.
...
nothing optional sees no difference. One that does gets to offer an extra
without having it approved by default, which is the only reading under which
"optional" means anything. `silo plugin info` prints both lists with the author's
reason beside each claim, and the admin UI shows the same thing beside a
checkbox.

A grant short of a required claim is a warning rather than a refusal, because
narrowing on purpose is a legitimate thing to do. It is a warning you will see,
on the start and on the change.

Both commands work offline, against the data directory. They hold the same
authority `silo keys create` already has there. That is what makes them the way
out of the boot deadlock, and the way to provision a plugin in CI.

Effective authority is the **union** of the two paths, each bounded by what the
manifest requested. There are two paths because they serve genuinely different
...
`silo plugin revoke` clears only the stored half, and it says so when
`silo.toml` still grants something.
...
sees it. The reason is not that a hostile plugin would gain anything by holding
...
**`If-Match` is required on everything that writes the record.** On a grant it
is not ceremony: approving means approving *what you read*. Without the fence, a
package whose request changed between the read and the approval would be
approved on the strength of the older one. `restart` and `rescan` write no
record, so neither takes one.

**The API never writes `silo.toml`.** An API that could add a `[[plugins]]`
block would be a code-execution primitive wearing a management claim. `rescan`
reads that file, the one the operator already wrote, and applies it: plugins
added, removed, reordered, upgraded in place, or reconfigured. It is also how a
grant made with the offline CLI reaches a server that is already running.

`enabled` is orthogonal to the grant. A disabled plugin keeps its claims and its
key, because pausing something is not the same decision as un-approving it. An
operator who had to re-approve after every pause would learn to approve widely
to avoid the trouble.

`PATCH .../config` takes an
[RFC 7396](https://www.rfc-editor.org/rfc/rfc7396) merge patch, so one setting
changes without restating the block, and `null` removes one. The result
**replaces** `silo.toml`'s block for that plugin rather than merging with it.
Two config documents have no sane join, and "what config is this plugin running
with" should be something you read, not something you compute.
`DELETE .../config` is the way back, and `config_source` on every view says
...
with hook delivery, and it flags a hook that can change or stop a write, because
...
`plugin.revoke`, `plugin.enable`, `plugin.disable` and `plugin.configure`. It
never records an entry write, which is what `rev`, `updated_at` and the hook
stream already are. The **services** append rather than the routes, so a change
made with the offline CLI is in it too. Retention is unbounded on purpose: an
authority log grows with decisions, not with traffic.

## Hooks

There are six. The five entry hooks each carry `op`, so `create` and `update`
share one function:
...
delete a 100k-event fan-out for a fact that is one sentence long.

So the event carries the collection, `erased`, and a `cause` of `collection`,
`environment` or `project`. The last two mean every sibling collection is going
too, so the useful reaction is to drop the scope rather than one table. There is
no `before` counterpart: a veto there would overrule an explicit `force` from a
caller who already had to hold `entries:delete` at that reach.
...
Plugins shape `data`. The envelope belongs to silo, and no hook can set it. That
means `id`, `rev`, `seq` and the timestamps.
...
rewriting data mid-import would make export then import non-idempotent. That is
the single property [Export, import and copy](transfer.md) rests on.

**Delivery is claim-checked**, before the event crosses into the worker. See
[What a plugin is allowed to do](#what-a-plugin-is-allowed-to-do).
...
Nothing is a `204`, a string is `text/plain`, and any other object is a JSON
body. `{ status, headers, body }` or `{ json }` sets one explicitly. Throwing
...
calling, never how they proved it.** It holds an id, a label and their claims,
and silo withholds `Authorization`, `X-Api-Key` and `Cookie`. It is `null` on a
`public` route reached with no credential.

Two things are worth being deliberate about.

**A route runs with the plugin's authority, not the caller's.** That is what a
plugin route is for, because a handler bounded by the caller's claims could only
do what the caller could have done directly. But it means **reaching a route is
reaching the plugin's grant**. Serving routes at all therefore costs the
`http:route` claim, which declaring one adds to the request for you.
`auth: "public"` is a separate line on the grant screen, because a public route
publishes whatever the plugin was granted to anyone who can reach the URL. Check
`request.caller.claims` when a route should be narrower than the plugin is.

**silo matches the routes, and a plugin never registers one.** The grammar is
literal segments and `:name` parameters. There are no wildcards and no regular
expressions, and a path that could reach outside the namespace is refused at
startup, naming the package. So a plugin cannot shadow or reorder a built-in
route, and its routes come and go with `enable`, `disable`, `grant`, `revoke`
and `rescan` without a restart, like its hooks.

`HEAD` reaches a declared `GET`. A handler that misses `timeout_ms` answers
`504`, and the plugin is left `failed` until
`POST /api/plugins/<name>/restart`.

**A route says what body it takes.** The default is text, capped at 1 MiB. A
route that receives a *file* declares bytes instead. It is handed
`request.bytes` undecoded, while `request.body` stays `null`:

```json
{ "method": "POST", "path": "/source", "body": { "kind": "bytes", "max_bytes": 67108864 } }
```

The cap is yours to declare and silo's to bound, at 64 MiB. It is in the
manifest because it is how much the host will allocate for whoever reaches the
route. An operator reads that number beside the route when they approve
`http:route`, and raising it in a later release asks them again.

Past the cap, silo **refuses** the request rather than truncating it. A plugin
cannot tell a body it was not given from one that was never sent, so the
alternative is a `200` describing work done on the wrong input.
...
It appears under **Settings > Plugins > your plugin**, below the grant. It runs
in an iframe with `sandbox="allow-scripts"` and **no** `allow-same-origin`, so
it has no origin of its own. `localStorage` throws, `document.cookie` is empty,
and nothing it fetches carries a credential.

That is not belt-and-braces. The admin keeps an API key for every server you
have configured, and a panel able to read them would hold more than any plugin
can be granted.
...
routes, and your handlers spend the plugin's grant. That is why none of your
routes has to be `public` for a screen to work.

The admin's theme arrives as CSS custom properties, so `var(--text)` and
`var(--accent)` follow whatever the operator has on.
...
same guards, and the same answers a key with those claims would get. That is not
an analogy for the claim check. It is the claim check. **A plugin is an API key
with code attached.**
...
// /api/, and a refusal comes back as a status, not a throw.
const response = await ctx.fetch("/api/media?limit=5");
```

Authority comes from two places, and it is the **union** of them: the `claims`
in `silo.toml`, and what an operator approved through
`PUT /api/plugins/{name}/grant` or `silo plugin grant`. Neither may exceed what
...
committed one. silo checks it **before the event crosses into the worker**,
...
the start. A missing API claim is not an error, because a plugin may run on less
than it asked for. But a hook that can never fire means the plugin loads, looks
...
live, and `deactivate(ctx)` on the way out. `activate` runs before silo takes
its first request, so setup that must succeed belongs there, and a throw refuses
the start and names the plugin. It costs no claim, because nobody but silo calls
it and its `ctx` is the same claim-checked surface a hook's is. What it adds is
work nothing asked for, not reach. `deactivate` is best-effort: the decision to
stop has been taken by the time it runs.
...
rejection**, and it surfaces as a `400` or a `403`. Any other throw is a
**plugin fault**, governed by `on_error`: `fail` refuses the write, and `skip`
logs it and carries on. Either way silo logs it. `afterWrite` and `afterDelete`
never fail a request, because the write has already committed and a `500` there
would invite a retry that writes twice.
...
reported while the server keeps serving, and silo does not restart it into the
same wall on the next write. It does *not* stop plugin code reading the database
or opening a socket. Worker code holds full privileges.
...
It **runs none of the package's code**, and no lifecycle script, ever. silo
validates the manifest, checks the `silo` range against your binary, and refuses
a provider that takes a reserved driver name, all before anything is imported.

Archives are refused if they contain absolute paths, `..`, symlinks, hard links,
device nodes, or setuid, setgid or sticky mode bits. silo checks an archive in
full before it writes a single file, so a bad package leaves nothing behind. An
ordinary executable at `0755` is fine: the mode check is about privilege, not the
executable bit.

What can be verified depends on where the package came from, and `add` tells you
which you got:
...
| git | nothing. It is pinned by resolved commit, and `--integrity` is refused |

Passing `--integrity` to npm is worth it when you know the digest
independently. The registry supplies both the tarball and the digest it is
checked against, so pinning is what a compromised registry cannot satisfy.

silo shows a plugin's claims before it grants them, and it asks you. That
distinction is the point: a manifest *requests* claims, and you *grant* them.
...
silo installs no dependencies. A plugin needs none, which is what `silo:api`
buys, and a package that declares some is installed with a warning rather than a
dependency tree.

There is no `remove`. `POST /api/plugins/{name}/disable` stops one on a running
server, deleting the `[[plugins]]` block stops it loading at all, and deleting
the directory is how you are rid of it.

A running server picks up an added plugin on `POST /api/plugins/rescan`, or at
its next start, and never on its own. Placing a directory under `plugins/` is
not consent to run it, and neither is listing it. The plugin still arrives
`pending`, and you grant it separately.
...
load, and both show the request beside the grant, as `[pending]`, `[granted]`,
`[needs_review]` or `[granted, disabled]`.

`doctor` answers "would `serve` start?" without starting a server. It exits
non-zero when the answer is no, including when a plugin would start and quietly
do nothing.
...
claims are simply not in it. silo deliberately does not advance the digest the
record was approved against while a review is outstanding, because a second
start would settle it in silence.

A plugin that fails to load **refuses the start**. That covers a missing
directory, a version range that excludes this binary, invalid config, a claim
that was not granted, and a declared hook the module does not export. silo never
skips one with a warning. An instance that runs, looks healthy, and has quietly
stopped doing what a plugin was installed to do is the worst outcome available.
