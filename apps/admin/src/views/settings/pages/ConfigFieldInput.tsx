...
export function ConfigFieldInput({
  ...
  const help = [field.help, field.zeroMeans ? `0 means ${field.zeroMeans}.` : '']
    .filter(Boolean)
    .join(' ')
  ...
  return (
    ...
    >
      {field.type === 'boolean' ? (
        ...
        >
          {/* The file may say nothing, and that is not the same as the first
              option. An empty choice keeps "unset" reachable, so a section can
              be put back to following the defaults. */}
      ...
      {inUse && (
        <span
          className={`${styles.controlNote} ${inUse.restart ? styles.controlNotePending : ''}`}
        >
