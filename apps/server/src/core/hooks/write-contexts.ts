 ...
 * `PluginContext`, which cannot use a constant because it extends the causal
 * chain, and the transfer paths deliberately do not dispatch at all (see
 * `Hooks`).
 *
...
export class WriteContexts {
  /** An ordinary request: nothing plugin-shaped caused it. */
  static readonly Api: WriteContext = { origin: "api", chain: [] };
}
