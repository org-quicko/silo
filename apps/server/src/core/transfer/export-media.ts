 ...
 * instance-global (§8.1) and blob keys are flat. What the mode changes is how
 * many of those keys are written at all — see §7.7 in
...
export class ExportMedia {
  ...
  static async write(options: {
    ...
    for (const key of keys) {
      ...
      if (!blob) continue;
      await options.sink.file(`media/${key}`, blob.data);
      written++;
    }
    return written;
  }
