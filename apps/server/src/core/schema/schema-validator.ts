...
export class SchemaValidator {
  ...
  constructor(store: Storage, options: SchemaValidatorOptions = {}) {
    this.store = store;
    this.allowRemoteRefs = options.allowRemoteRefs === true;
  }
  ...
  async checkSchemaDoc(scope: Scope, collection: string, raw: any): Promise<void> {
    ...
    try {
      ...
      if (ValidationError.is(caught)) {
        throw caught;
      }
      throw new ValidationError(`invalid JSON Schema: ${SchemaValidator.refError(caught)}`);
    }
  }
  ...
  async validateEntry(scope: Scope, collection: string, data: any): Promise<void> {
    ...
    if (!validateFn) {
      ...
      try {
        ...
        throw new Error(`compiling schema for "${collection}": ${SchemaValidator.refError(caught)}`);
      }
    }
  ...
  private async compileFrom(all: Map<string, any>, collection: string) {
    ...
    for (const [name, schema] of all.entries()) {
      try {
        ...
        if (!caught?.message?.includes("already exists")) {
          throw caught;
        }
      }
    }
