...
export class Daemon {
   ...
   *
   * Two shapes to cover: a source run, where argv[1] is the entry script and
   * has to be passed on, and a compiled binary, where argv[1] is a path inside
   * Bun's virtual filesystem. The second must never be forwarded — the child
   * gets its own, so a second copy would arrive as argv[2] and the CLI would
   * take it for a subcommand.
   *
   * `realpathSync` is what separates the two: inside a compiled binary
   * `existsSync` and `statSync` both answer **true** for the virtual path, and
   * only resolving against the real filesystem fails there.
   *
   * Parameterized because the compiled shape is otherwise only reachable by
   * building a binary; the tests pass both argv shapes in directly.
   */
