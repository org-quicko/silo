...
export class ServeDetachedCommand {
  ...
  static async run(config: Config, version: string): Promise<void> {
    await RunFile.assertNotRunning(config.storage.path);

    const firstRun = !fs.existsSync(config.storage.path);
    const logFile = LogLocation.forDetached(config);
    ...
    const extraArgs = config.log.file ? [] : ["--log-file", logFile];

    const pid = Daemon.spawnDetached(logFile, extraArgs);

    const started = await ServeDetachedCommand.awaitRunFile(config.storage.path, pid, Daemon.StartTimeoutMs);
    if (!started) {
      ServeDetachedCommand.report(`the background server (pid ${pid}) did not come up on ${config.listen}`, logFile);
    }
    ...
    console.log(`  data    ${config.storage.path}`);
