 ...
 *
 * The root `package.json` is the single source of truth at runtime (D28) — the
 ...
 * remainder: their versions are inert but not invisible, and a reader cannot
 * tell an inert version from a stale one, so they move together.
 *
...
export class SetVersion {
   ...
   *
   * `create-silo-plugin` is here for a reason the others are not. Their
   * versions are inert; the scaffolder's is *load-bearing*, because `SiloRange`
   * derives the `"silo"` range it writes into every scaffolded manifest from it
