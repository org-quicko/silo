...
describe("schema access", () => {
  ...
  test("clearing removes the keyword rather than writing false", () => {
    ...
    SchemaAccess.setRequiresAuth(document, true);
    expect(document).toEqual({ "x-silo-auth": true });
    SchemaAccess.setRequiresAuth(document, false);
    expect(Object.hasOwn(document, "x-silo-auth")).toBe(false);
  });

  test("the writer and the predicate agree", () => {
    const document: Record<string, unknown> = {};
    SchemaAccess.setRequiresAuth(document, true);
    expect(SchemaAccess.requiresAuth(document)).toBe(true);
    SchemaAccess.setRequiresAuth(document, false);
    expect(SchemaAccess.requiresAuth(document)).toBe(false);
  });
});
