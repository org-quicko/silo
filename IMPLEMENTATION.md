...
{ "project": "acme", "env": "prod", "collection": "posts",
  ...
  "snippets": [ { "path": "$.data.body", "text": "…our [pricing] page…" } ] }
```
...
6. **Search (in progress)** — D30/§5.5: a `Searcher` port with a portable `ScanSearcher` on every adapter and `SqliteSearcher` (FTS5) where the build has it. Addressing is D29 JSONPath, shared with filters and sort.
